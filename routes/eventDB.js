...
const eventSchema = new mongoose.Schema({
    ...
    discount: {
        type: Number,
        required: function () { return this.isValid }
    }
})
...
// 
async function createEventInDB(req,id) {
    const duplicate = await Event.findById(id);
    return new Promise((result, reject) => {
        if (!duplicate)
            result(new Event({ name: req.name }).save())
        else
...
async function getEventFromDB(id) {
    ...
    return new Promise((result, reject) => {
        if (event)
            ...
            reject(new Error("no event with id "+id+" found"))
    });
...
async function deleteFromDB(id) {
    return new Promise((result, reject) => {
        ...
        if (!oldEvent)
            ...
            result(Event.findByIdAndRemove(id))
...
async function updateInDB(req,id) {
    return new Promise((result, reject) => {
        if (!Event.findById(id))
            ...
            result(Event.findByIdAndUpdate(id, {
                $set: {
                    name: req.name,
...
module.exports = { getEventFromDB, getEventsFromDB, deleteFromDB, updateInDB, createEventInDB, eventSchema};